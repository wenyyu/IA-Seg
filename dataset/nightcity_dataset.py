...
class nightcityDataSet(data.Dataset):
    def __init__(self, args, root, list_path, max_iters=None, set='val'):
        ...
        nightcity_transform_list = [joint_transforms.RandomSizeAndCrop(args.input_size, False, pre_size=None,
                                                                       scale_min=0.5, scale_max=1.0, ignore_index=255),
                                    joint_transforms.Resize(args.input_size),
                                    joint_transforms.RandomHorizontallyFlip()]
        self.joint_transform = joint_transforms.Compose(nightcity_transform_list)
        ...
        for name in self.img_ids:
            ...
            label_file = osp.join(self.root, "label/train/%s" % name ).replace(".png", "_labelIds.png")
            self.files.append({
                "img": img_file,
                "label": label_file,
                "name": name
